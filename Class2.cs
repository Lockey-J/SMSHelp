...
namespace SMSHelp
{
    ...
    public sealed class SqliteConn
    {
        ...
        private static readonly Dictionary<string, SQLiteConnection> connPool =
          new Dictionary<string, SQLiteConnection>();
        private static readonly Dictionary<string, ReaderWriterLock> rwl =
          new Dictionary<string, ReaderWriterLock>();
        private static readonly SqliteConn instance = new SqliteConn();
        private static readonly string DEFAULT_NAME = "LOCAL";
        #region Init
        // 使用单例，解决初始化与销毁时的问题
        private SqliteConn()
        {
        ...
        private static SQLiteConnection CreateConn(string dbName)
        {
            ...
            try
            {
                ...
                {
                    DataSource = Environment.CurrentDirectory + dbName
                };
